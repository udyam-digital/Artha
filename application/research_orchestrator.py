...
from application.research_utils import coerce_string_list, extract_tagged_json, extract_text, unique_payload_key
...
from persistence.store import load_latest_mf_snapshot, load_latest_portfolio_snapshot
...
class DeepResearchOrchestrator:
    def __init__(self, settings: Settings | None = None, client: AsyncAnthropic | None = None):
        ...
        framework = (SKILLS_DIR / "equity_analysis.md").read_text(encoding="utf-8")
        self.equity_framework = framework if "You are Artha" in framework else f"You are Artha.\n\n{framework}"
    ...
    def _extract_tagged_json(self, raw_text: str, tag_name: str, identifier: str) -> dict[str, Any]:
        return extract_tagged_json(raw_text, tag_name, identifier, ResearchExecutionError)

    async def research_latest_snapshots(self) -> tuple[ResearchDigest, Path, list[Path], Path]:
        return await self.research_snapshots(
            load_latest_portfolio_snapshot(self.settings), self._load_latest_mf_snapshot_optional()
        )

    async def research_snapshots(
        self, portfolio_snapshot: PortfolioSnapshot, mf_snapshot: MFSnapshot | None
        ...
        for kind, result in results:
            if isinstance(result, Exception):
                ...
                continue
            per_holding_payloads[unique_payload_key(kind, result.identifier, per_holding_payloads)] = result.model_dump(
                mode="json"
            )
            if kind == "equity":
        ...
        digest = ResearchDigest(
            ...
            portfolio_digest=await self._build_digest_text(equity_reports, mf_reports, errors),
            errors=errors,
        )
        from application import research as research_module

        digest_path, holding_paths, index_path = research_module.save_research_digest(
            digest,
    ...
    def _load_latest_mf_snapshot_optional(self) -> MFSnapshot | None:
        ...
        return None if not path.exists() else load_latest_mf_snapshot(self.settings)

    async def _research_equity_holding(self, holding: Holding) -> EquityResearchArtifact:
        raw_text = await self._run_tool_loop(
            system=f"{self.equity_framework}\n\n{self.portfolio_rules}\n\nYou are a dedicated Artha sub-agent researching exactly one Indian equity holding. Keep searching until you have enough current evidence to produce a confident, source-cited report.",
            user_prompt=(
                f"Research this Indian equity holding deeply: {holding.tradingsymbol} on {holding.exchange}. "
                f"Current portfolio weight: {holding.current_weight_pct:.2f}%. Target weight: {holding.target_weight_pct:.2f}%. "
                f"Use tavily_search up to {self.settings.analyst_max_searches} times across Screener, latest results, concalls, investor presentations, exchange filings, and recent sector/company news. "
                "Return exactly one JSON object wrapped in <equity_research>...</equity_research> tags with keys: identifier, title, data_freshness, sources, bull_case, bear_case, what_to_watch, red_flags, confidence_summary."
            ),
            label=f"research_equity:{holding.tradingsymbol}",
            metadata={"phase": "research_equity", "ticker": holding.tradingsymbol},
        )
        payload = extract_tagged_json(raw_text, "equity_research", holding.tradingsymbol, ResearchExecutionError)
    ...
    async def _research_mf_holding(self, holding: MFHolding) -> MFResearchArtifact:
        raw_text = await self._run_tool_loop(
            system="You are a dedicated Artha sub-agent researching exactly one Indian mutual fund holding. Keep searching until you have enough current evidence to produce a confident, source-cited report. Focus on category, mandate, style, concentration, expense ratio, AUM, overlap risk, and recent fund commentary. Do not analyze a mutual fund like a single stock.",
            user_prompt=(
                f"Research this Indian mutual fund deeply: {holding.fund}. Scheme type: {holding.scheme_type or 'Unknown'}. Plan: {holding.plan or 'Unknown'}. "
                f"Current value: INR {holding.current_value:.2f}. Use tavily_search up to {self.settings.analyst_max_searches} times across AMC pages, Value Research, Morningstar if available, factsheets, portfolio disclosures, and recent fund commentary. "
                "Return exactly one JSON object wrapped in <mf_research>...</mf_research> tags with keys: identifier, title, data_freshness, sources, fund_house, category, mandate, portfolio_style, expense_ratio_note, aum_note, overlap_risk, recent_commentary, risks, confidence_summary."
            ),
            label=f"research_mf:{holding.tradingsymbol or holding.fund}",
            metadata={"phase": "research_mf", "fund": holding.fund, "tradingsymbol": holding.tradingsymbol},
        )
        payload = extract_tagged_json(raw_text, "mf_research", holding.fund, ResearchExecutionError)
        return MFResearchArtifact(
            generated_at=datetime.now(UTC),
    ...
    async def _run_tool_loop(self, *, system: str, user_prompt: str, label: str, metadata: dict[str, Any]) -> str:
                        ...
                        tool_results.append(
                            {
                                ...
                                "content": json.dumps(
                                    {
                                        "error": f"tavily_search budget exhausted; max {self.settings.analyst_max_searches} searches allowed."
                                    },
                                    ensure_ascii=True,
                                ),
                                "is_error": True,
    ...
    async def _build_digest_text(
        self, equity_reports: list[EquityResearchArtifact], mf_reports: list[MFResearchArtifact], errors: list[str]
    ) -> str:
        messages = [
            {
                "role": "user",
                "content": (
                    "Write a concise portfolio research digest for an Indian investor using the supplied holding reports. Cover major strengths, concentration risks, and what deserves follow-up. "
                    f"Input JSON:\n{json.dumps({'equity_reports': [report.model_dump(mode='json') for report in equity_reports], 'mf_reports': [report.model_dump(mode='json') for report in mf_reports], 'errors': errors}, ensure_ascii=True)}"
                ),
            }
        ]
        log_estimated_input_tokens(label="[research_digest]", messages=messages)
        response = await self.client.messages.create(
            model=self.settings.model, max_tokens=self.settings.summary_max_tokens, messages=messages
        )
        record_anthropic_usage(
            settings=self.settings,
        ...
        return extract_text(response) or "No research digest generated."
    ...
    async def _run_research_jobs(
        ...
        async def run_job(
            kind: str, _identifier: str, factory: Any
        ...
        return await asyncio.gather(
            *(asyncio.create_task(run_job(kind, identifier, factory)) for kind, identifier, factory in jobs)
        )
