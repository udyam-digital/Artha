...
from tools import KiteMCPClient, ToolExecutionError, kite_get_portfolio, load_kite_server_definition
...
async def handle_run(args: argparse.Namespace) -> int:
    ...
    if args.rebalance_only:
        async with KiteMCPClient(load_kite_server_definition(settings)) as kite_client:
        ...
        report = await agent.run(ticker=args.ticker, console_filename=args.with_console)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_parser.add_argument(
        "--with-console",
        help="Read a Zerodha Console export CSV from data/console_exports for tax-aware analysis",
    )
